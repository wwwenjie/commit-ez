...
export const fetchCardOptions = async () => {
  let { site, email, token } = config.get(id) || {}
  ...
  const response = await fetch(`https://${site}/rest/api/3/search?jql=assignee=currentuser()`, {
    method: 'GET',
    headers: {
...
export const setToken = async () => {
  ...
  const { site, email, token } = await prompts([{
    type: 'text',
    name: 'site',
    message: 'Input jira site For example, your-domain.atlassian.net',
