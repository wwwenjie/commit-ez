...
import { config, configFormat, configIntegrations, configUsername } from './config.js'
...
export const commit = async ({ scope, staged, deep, push }) => {
  ...
  if (isTwStyle && !config.has('integrations')) {
    await configIntegrations()
  }
  const integrationIds = config.get('integrations') || []
  ...
  if (isChooseHistory) {
    ...
    response.cardName = get(find(cardOptions, { value: response.cardChoice }), 'description')
  }
