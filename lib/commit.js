...
export const commit = async ({ scope, staged, deep, push }) => {
  ...
  if (map(integrations, 'id').includes(response.cardChoice)) {
    ...
    const spinner = ora(`Loading cards from ${integration.displayName}`).start()
...
export const outputHistory = ({ deep }) => {
  ...
  orderBy(history, 'date', 'desc').slice(0, deep).forEach(h => {
    // todo: add format in history, generate commit message in formats
    const hasCardNumber = h.card.number
    console.log(hasCardNumber ?
      `${new Date(h.date).toLocaleString()}  ${getCommitMessage(h)}` :
      `${new Date(h.date).toLocaleString()}  ${getCommitMessage(h)}`)
  })
}
...
function getCommitMessage ({ type, scope, description, username, card }) {
  const format = config.get('format')

  const scopeMessage = scope ? `(${scope})` : ''

  if (format === FORMAT.THOUGHTWORKS) {
    return `[${username}] #${card.number} ${type}${scopeMessage}: ${description}`
  }


  if (format === FORMAT.CONVENTIONAL) {
    return `${type}${scopeMessage}: ${description}`
  }
}
