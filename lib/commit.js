...
const { orderBy, uniqBy, find, last, get, map } = _
...
export const commit = async ({ staged, deep }) => {
  ...
  if (_.map(integrations, 'id').includes(response.cardChoice)) {
    const integration = _.find(integrations, { id: response.cardChoice })
